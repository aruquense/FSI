...
class ramificacionacotacion(Queue):
    """copia de fifoqueue, porque siempre sale el primero, que en este caso es el de valor mas bajo"""
    """la entrada no sera colocada en primera posicion, sino que se reordenara"""
    """A ramificacion-acotacion Queue."""
    ...
    def extend(self, items):
        ...
        self.A.extend(items)
        self.A.sort(key=lambda self: self.path_cost)
...
class ramificacionacotacionconsubestimacion(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda t: t.path_cost+self.problem.h(t))
