...
def getGAction(state):
    if max(Q[state]) != 0:
        #print np.argmax(Q[state])
        #print actions_list.values().index(np.argmax(Q[state]))
        #print

        return list(actions_list)[(actions_list.values().index(np.argmax(Q[state])))]
    return random.choice(getActions(state))

def getEGAction(state):
    if max(Q[state]) != 0:
...
for i in xrange(500):
    ...
    while state != final_state:
        numerodeacciones=numerodeacciones+1
        #action = getRndAction(state)
        #action = getGAction(state)
        action = getEGAction(state)
...
# numero total de acciones / 100 porque xrange=100
print (numerodeacciones/500)
